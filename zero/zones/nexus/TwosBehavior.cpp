...
std::unique_ptr<behavior::BehaviorNode> TwosBehavior::CreateTree(behavior::ExecuteContext& ctx) {
  ...
  //
  // That is the whole "retreats but never recovers" problem. Multifire is the case that actually
  // inverts the sign, and it is also worthless here - a spread exists to cover ground against a
  // cluster, and a retreat is aimed at the one chaser behind us. So multifire is forced off while the
  // recharge timer is running, below.
  //
  // Single fire stays on, deliberately. This branch exists because a silent retreat produced a death
  // spiral in rec17 (outnumbered -> permanent retreat -> no return fire -> still outnumbered, 12-0),
  ...
  builder
    .Selector()
        .Sequence() //Join the queue first thing and auto join TODO: add command or checks to requeue if something goes wrong later such as recycled arena
            ...
            .Child<ChatMessageNode>(ChatMessageNode::Public("?next 2v2pub"))
                    ...
                    .Sequence() // Aim at target and shoot while seeking them.
                        ...
                        .Parallel()
                            ...
                            .Sequence(CompositeDecorator::Success) // PB thor fire check.
                                ...
                                .Child<RelativeShotVelocityNode>(WeaponType::Thor, "thor_fire_velocity")
